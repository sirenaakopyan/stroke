...
def plot_historgram_with_cate(df, dir, cate):
    sns.kdeplot(data=df[df['stroke'] == 0], x=cate, shade=True,  alpha=1,
                label="No Stroke")
    sns.kdeplot(data=df[df['stroke'] == 1], x=cate, shade=True,  alpha=0.8,
                label="Stroke")
    plt.legend()
...
def plot_further_explore_age(df):
    df = df.copy()
    df['age_cat'] = pd.cut(df['age'], bins=[0, 13, 18, 45, 60, 200],
...
def data_transformation_correlation(df):
    # feature log transformations (since data is skewed ->
    # transform back to normal distribution, would not affect the correlation)
    df['age'] = df['age'].apply(lambda x: np.log(x+10)*3)
    df['avg_glucose_level'] =\
        df['avg_glucose_level'].apply(lambda x: np.log(x+10)*2)
...
def plot_correlation(df):
    ...
    yticks = ['Age', 'Hyper tension', 'Heart Disease', 'Marriage', 'Work',
              'Residence', 'Glucose Level', "BMI", 'Smoking Status', 'Stroke']
    xticks = ['Gender', 'Age', 'Hyper tension', 'Heart Disease', 'Marriage',
              'Work', 'Residence', 'Glucose Level', "BMI", 'Smoking Status']
    ax.set_yticklabels(yticks, {'font': 'serif', 'size': 10,
                                'weight': 'bold'}, rotation=0, alpha=0.9)
    ax.set_xticklabels(xticks, {'font': 'serif', 'size': 10,
                                'weight': 'bold'}, rotation=90, alpha=0.9)
